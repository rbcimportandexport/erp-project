      ...
      <div className="rounded-md bg-white p-5 shadow-sm">
        <div className="mb-5 flex flex-col gap-3 md:flex-row">
          ...
          <div className="flex items-end gap-2">
            ...
            {hasEditPermission && <Button onClick={save}>Save</Button>}
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {fields.map((field) => (
            <Input 
              key={field} 
              label={field} 
              type={field.includes("Amount") || field === "paidAmount" ? "number" : "text"} 
              value={payment[field] || ""} 
              onChange={(event) => setPayment((value) => ({ ...value, [field]: event.target.type === "number" ? Number(event.target.value) : event.target.value }))} 
